...
{
    [ 
        iotdb_api_username: "p46dbdd50d50915bbccde79bcabb8efd2",
        iotdb_api_key: "a1913bd1-7949-48ed-bb53-f9e7223e441c"
    ]
}
...
preferences {
    section("Allow IOTDB to Control & Access These Things...") {
        ...
        input "d_relativeHumidityMeasurement", "capability.relativeHumidityMeasurement", title: "relativeHumidity", required: false, multiple: true
		input "d_illuminanceMeasurement", "capability.illuminanceMeasurement", title: "illuminance", multiple: true
        input "d_kwh", "capability.powerMeter", title: "power", required: false, multiple: true
    }
}
...
def _event_subscribe()
{
    ...
    subscribe(d_illuminanceMeasurement, "illuminance", "_on_event")
    subscribe(d_kwh, "power", "_on_event")
}
...
//    log.debug "_on_event XXX event.id=${evt?.id} event.deviceId=${evt?.deviceId} event.isStateChange=${evt?.isStateChange} event.name=${evt?.name}"
    ...
    log.debug "_on_event (device=${dt.device}) deviceId=${jd}"
...
def _api_list()
{	
    log.debug("params.type is ${params.type}: ${it}")
...
def _dtd()
{
    [ 
        ...
        illuminance: d_illuminanceMeasurement,
        power: d_kwh,
...
private _device_to_json(device, type) {
    ...
    log.debug("returning json for ${type}")
    def vd = [:]
